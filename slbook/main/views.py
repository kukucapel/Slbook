...
def main(request, number_page = 1):
    ...
    if articles_all.count() > count_news_on_page:
        ...
        return render(request, 'index_main.html', {"articles":articles, "current_date":current_date,"count_page":count_page, "pages":pages_section, "rightNews": showRightPanel(), "minus": minus, "plus":plus, "last":count_page, "current_page":number_page})

    return render(request, 'index_main.html', {"articles":articles_all, "current_date":current_date, "rightNews": showRightPanel()})
...
def show_post(request, post_id = None):
    ...
    current_date = timezone.now()
    return render(request, 'index_main.html', {"articles":articles, "post_id":post_id, "current_date": current_date, "rightNews": showRightPanel()})